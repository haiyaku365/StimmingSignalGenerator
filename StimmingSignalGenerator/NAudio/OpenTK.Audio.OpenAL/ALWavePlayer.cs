...
namespace StimmingSignalGenerator.NAudio.OpenTK.Audio.OpenAL
{
    class ALWavePlayer : IWavePlayer
    {
        ...
        public void Init(IWaveProvider waveProvider)
        {
            ...
            sourceALFormat = OutputWaveFormat.ToALFormat();
            bufferSizeByte = OutputWaveFormat.ConvertLatencyToByteSize(DesiredLatency);
        ...
        private void ReadAndQueueBuffers(int[] _alBuffers)
        {
            for (int i = 0; i < _alBuffers.Length; i++)
            {
                ...
                AL.BufferData(_alBuffers[i], sourceALFormat, sourceBuffer, OutputWaveFormat.SampleRate);
                CheckAndRaiseStopOnALError();
